...
class Organism:
        ...
        if self.pregnant:
            if time.time() - self.time_pregnant > self.lifetime / 10:
                ...
                while self.baby is not None:
                    ...
                    print('Organism birth, population: ', len(population))
                    self.fertile = False
                    self.pregnant = False
                    self.time_pregnant = 0
                    if random.uniform(0, 5) > self.food_eaten / (50 * baby_count) or baby_count > 10:
                        self.baby = None
                    baby_count += 1
    ...
    def perceive(self, population):
        ...
        for organism in population:
            if self.get_dist(organism) <= self.perception * 7:
                ...
                elif organism.__class__ == Organism and organism.__class__ != Predator \
                        and organism not in self.organism_perceptions:
    ...
    def make_baby(self, mate):
        ...
        child.lifetime = (self.strength + self.endurance) * 2
    ...
    def herd(self):
        ...
        if self.hungry:
            self.find_food()
        elif len(self.predator_perceptions) > 0 and random.uniform(0, 5) < self.sociability:
            self.target = random.choice(self.predator_perceptions)
        elif len(self.predator_perceptions) > 0 and self.fertile and random.uniform(0, 5) < (self.endurance / 50):
            self.find_mate()
        else:
            self.target = None
...
class Predator(Organism):
    def __init__(self, x_pos=None, y_pos=None):
        ...
        if x_pos is not None and y_pos is not None:
            ...
            x_pos = int(random.uniform(20, 980))
    ...
    def move(self, width, height):
        if random.uniform(0, 50) <= 10:
            ...
            # if the organism is herding, move randomly with the herd
                ...
                if self.get_dist(self.target) <= self.perception * 10:
                    delta_x, delta_y = self.move_randomly()
                else:
                    delta_x, delta_y = self.move_towards_target()
            # if the organism is not herding, move directly towards the target
    ...
    def perceive(self, population):
        ...
        self.prey_perceptions = []
        for organism in population:
            if self.get_dist(organism) <= self.perception * 10:
                if organism.__class__ == Plant and organism not in self.plant_perceptions:
                    self.plant_perceptions.append(organism)
    ...
    def can_eat(self, target):
        if not target.herding and self.get_dist(target) < self.bounds.width / 4 and self.bounds.width > target.bounds.width:
            return True
        return False
    ...
    def get_older(self, population):
        ...
        if self.pregnant:
            if time.time() - self.time_pregnant > self.lifetime / 10:
                ...
                population.append(self.baby)
                self.children.append(self.baby)
                print('Predator birth, population: ', len(population))
                self.fertile = False
                self.pregnant = False
                self.baby = None
                self.time_pregnant = 0
    ...
    def find_mate(self):
        self.foraging = False
        self.herding = False
        if len(self.predator_perceptions) > 0:
            self.focus_time = time.time()
            self.target = random.choice(self.predator_perceptions)
            self.reproducing = True
            if self.sex == 'M' and self.target.sex != 'F':
